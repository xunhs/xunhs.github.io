...
function executeSearch(term) {
  ...
  if (results.length === 0) { // no results based on what was typed into the input box
    ...
    for (let item in results) { // only show first 5 results
        ...
        if (permalinks.includes(results[item].item.uri)) {
            continue;
        }
    //   console.log('item: %d, title: %s', item, results[item].item.title)
      searchitems = searchitems + '<li><a href="' + results[item].item.uri + '" tabindex="0">' + '<span class="title">' + results[item].item.title + '</span></a></li>';
