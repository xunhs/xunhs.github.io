...
tags:
  ...
  - 深度学习
abbrlink: 549e529e
date: 2020-07-17T09:28:41+00:00
---

> 拷贝自[Github-StyleTransferTrilogy](https://github.com/CortexFoundation/StyleTransferTrilogy)。Github上刷到比较好玩的风格迁移的代码和讲解，并做部分笔记


<!--more-->


# 风格迁移三部曲 (Style Transfer Trilogy)
...
<u>经过修改的 VGG16 可以输出 relu1_2,relu2_2,relu3_3,relu4_3 这几个特定层的特征图</u>。下面这两句代码就是它的用法：
...
![](https://cdn.jsdelivr.net/gh/xunhs/image_host/images/2020/7/1595063136.png)

提示：在本方法中没有 Image Transform Net，为了表述方便，我们使用了第二篇论文中的图。

这里使用的损失函数是：

![](https://latex.codecogs.com/svg.latex?\Large\ell^{\phi,j}_{feat}(\hat{y},y)=\frac{1}{C_jH_jW_j}||\phi_j(\hat{y})-\phi_j(y)||^2_2)  
其中：

* ![](https://latex.codecogs.com/svg.latex?\hat{y})是输入图像（也就是生成的图像）
* ![](https://latex.codecogs.com/svg.latex?y)是内容图像
* ![](https://latex.codecogs.com/svg.latex?\phi) 代表 VGG16
* ![](https://latex.codecogs.com/svg.latex?\j) 在这里是 relu3_3
* ![](https://latex.codecogs.com/svg.latex?\phi_j(x))指的是 x 图像输入到 VGG 以后的第 j 层的特征图
* ![](https://latex.codecogs.com/svg.latex?C_j\times&space;H_j\times&space;W_j)是第 j 层输出的特征图的尺寸

根据生成图像和内容图像在  relu3_3输出的特征图的均方误差（Mean Squared Error）来优化生成的图像与内容图像之间的内容一致性。
...
![](https://latex.codecogs.com/svg.latex?\Large{G^\phi_j(x)_{c,c'}=\frac{1}{C_jH_jW_j}&space;\sum_{h=1}^{H_j}&space;\sum_{w=1}^{W_j}&space;\phi_j(x)_{h,w,c}\phi_j(x)_{h,w,c'}})

其中：

* ![](https://latex.codecogs.com/svg.latex?\hat{y})是输入图像（也就是生成的图像）
* ![](https://latex.codecogs.com/svg.latex?y)是风格图像
* ![](https://latex.codecogs.com/svg.latex?C_j\times&space;H_j\times&space;W_j)是第 j 层输出的特征图的尺寸。
* ![](https://latex.codecogs.com/svg.latex?G^\phi_j(x))指的是 x 图像的第 j 层特征图对应的 Gram 矩阵，比如 64 个卷积核对应的卷积层输出的特征图的 Gram 矩阵的尺寸是 ![](https://latex.codecogs.com/svg.latex?(64,64))。
* ![](https://latex.codecogs.com/svg.latex?G^\phi_j(x)_{c,c'}) 指的是 Gram 矩阵第 ![](https://latex.codecogs.com/svg.latex?(c,c')) 坐标对应的值。
* ![](https://latex.codecogs.com/svg.latex?\phi_j(x))指的是 x 图像输入到 VGG 以后的第 j 层的特征图，![](https://latex.codecogs.com/svg.latex?\phi_j(x)_{h,w,c}) 指的是特征图 ![](https://latex.codecogs.com/svg.latex?(h,w,c))坐标对应的值。

Gram 矩阵的计算方法其实很简单，Gram 矩阵的 ![](https://latex.codecogs.com/svg.latex?(c,c')) 坐标对应的值，就是特征图的第 ![](https://latex.codecogs.com/svg.latex?c) 张和第 ![](https://latex.codecogs.com/svg.latex?c') 张图对应元素相乘，然后全部加起来并且除以 ![](https://latex.codecogs.com/svg.latex?C_j\times&space;H_j\times&space;W_j) 的结果。根据公式我们可以很容易推断出 Gram 矩阵是对称矩阵。
...
![](https://latex.codecogs.com/svg.latex?\Large\ell^{\phi,j}_{style}(\hat{y},y)=||G^\phi_j(\hat{y})-G^\phi_j(y)||^2_F)

其中：

* ![](https://latex.codecogs.com/svg.latex?\hat{y})是输入图像（也就是生成的图像）
* ![](https://latex.codecogs.com/svg.latex?y)是风格图像
* ![](https://latex.codecogs.com/svg.latex?G^\phi_j(x))指的是 x 图像的第 j 层特征图对应的 Gram 矩阵
...
![](https://cdn.jsdelivr.net/gh/xunhs/image_host/images/2020/7/1595062870.png)
...
![](https://cdn.jsdelivr.net/gh/xunhs/image_host/images/2020/7/1595835499.png)
...
![](https://cdn.jsdelivr.net/gh/xunhs/image_host/images/2020/7/1595063112.png)
...
> The output images are regularized with total variation regularization with a strength of between ![](https://latex.codecogs.com/svg.latex?1\times10^{-6}) and ![](https://latex.codecogs.com/svg.latex?1\times10^{-4}), chosen via cross-validation per style target.
...
> Total Variation Regularization. To encourage spatial smoothness in the output image ![](https://latex.codecogs.com/svg.latex?\hat{y}), we follow prior work on feature inversion [6,20] and super- resolution [48,49] and make use of total variation regularizer ![](https://latex.codecogs.com/svg.latex?\ell_{TV}(\hat{y})).

论文中提到了一个 TV Loss，这是为了平滑图像。它的计算方法很简单：

![](https://latex.codecogs.com/svg.latex?\Large{V_\text{aniso}(y)=\sum_{i,j}|y_{i+1,j}-y_{i,j}|+|y_{i,j+1}-y_{i,j}|})
...
![](https://cdn.jsdelivr.net/gh/xunhs/image_host/images/2020/7/1595062900.png)

可以看到对于任意内容图片，转换网络都能转换为固定风格的图像。根据下面这段代码进行的测速，1080ti 可以在4.82秒内完成 1000 张图像的风格迁移，相当于207fps，可以说是具有了实时性：

![](https://cdn.jsdelivr.net/gh/xunhs/image_host/images/2020/7/1595063091.png)
...
![](https://latex.codecogs.com/svg.latex?\large{\min_I\left(\lambda_c||\mathbf{CP}(I;w_f)-\mathbf{CP}(I_c;w_f)||^2_2+\lambda_s||\mathbf{SP}(I;w_f)-\mathbf{SP}(I_s;w_f)||^2_2\right)})

其中：

* ![](https://latex.codecogs.com/svg.latex?\mathbf{CP}) 是内容损失函数
* ![](https://latex.codecogs.com/svg.latex?\mathbf{SP}) 是风格损失函数
* ![](https://latex.codecogs.com/svg.latex?\lambda_c) 是内容权重
* ![](https://latex.codecogs.com/svg.latex?\lambda_s) 是风格权重
* ![](https://latex.codecogs.com/svg.latex?w_f) 是VGG16的固定权值
* ![](https://latex.codecogs.com/svg.latex?I_s) 是风格图像
* ![](https://latex.codecogs.com/svg.latex?I_c) 是内容图像
* ![](https://latex.codecogs.com/svg.latex?I) 是输入图像

那么通过<u>对输入图像 ![](https://latex.codecogs.com/svg.latex?I) 进行训练</u>，我们能够得到固定风格、固定内容的风格迁移图像。

## 情况2

![](https://latex.codecogs.com/svg.latex?\large{\min_w\sum_{I_c}\left(\lambda_c||\mathbf{CP}(I_w;w_f)-\mathbf{CP}(I_c;w_f)||^2_2+\lambda_s||\mathbf{SP}(I_w;w_f)-\mathbf{SP}(I_s;w_f)||^2_2\right)})

其中：

* ![](https://latex.codecogs.com/svg.latex?I_w) 是生成图像，![](https://latex.codecogs.com/svg.latex?I_w=\mathcal{N}(I_c;w),\mathcal{N}) 是图像转换网络

通过<u>对权值的优化</u>，我们可以得到一个<u>快速风格迁移模型</u>，它能够对任何内容图像进行风格转换，输出同一种风格的风格迁移图像。

## 情况3

![](https://latex.codecogs.com/svg.latex?\large{\min_\theta\sum_{I_c,I_s}\left(\lambda_c||\mathbf{CP}(I_{w_\theta};w_f)-\mathbf{CP}(I_c;w_f)||^2_2+\lambda_s||\mathbf{SP}(I_{w_\theta};w_f)-\mathbf{SP}(I_s;w_f)||^2_2\right)})

* ![](https://latex.codecogs.com/svg.latex?\theta) 是 ![](https://latex.codecogs.com/svg.latex?Meta\mathcal{N}) 的权值
* ![](https://latex.codecogs.com/svg.latex?w_\theta) 是转换网络的权值，![](https://latex.codecogs.com/svg.latex?w_\theta=Meta\mathcal{N}(I_s;\theta))，所以我们可以说转换网络的权值是 MetaNet 通过风格图像生成的。
* ![](https://latex.codecogs.com/svg.latex?I_{w_\theta}) 是转换网络生成的图像，![](https://latex.codecogs.com/svg.latex?I_{w_\theta}=\mathcal{N}(I_c;w_\theta))

总的来说就是风格图像输入 ![](https://latex.codecogs.com/svg.latex?Meta\mathcal{N}) 得到转换网络 ![](https://latex.codecogs.com/svg.latex?\mathcal{N})，转换网络可以将任意内容图像进行转换。通过输入大量风格图像和内容图像 ![](https://latex.codecogs.com/svg.latex?\sum_{I_c,I_s})，可以训练出能够产出期望权值的 ![](https://latex.codecogs.com/svg.latex?Meta\mathcal{N})。该模型可以输入任意风格图像，输出情况2中的迁移模型，进而实现任意风格任意内容的风格迁移。

## 转换网络（TransformNet）

![](https://cdn.jsdelivr.net/gh/xunhs/image_host/images/2020/7/1595062959.png)
...
![](https://cdn.jsdelivr.net/gh/xunhs/image_host/images/2020/7/1595063020.png)
...
![](https://cdn.jsdelivr.net/gh/xunhs/image_host/images/2020/7/1595063058.png)
...
![](https://cdn.jsdelivr.net/gh/xunhs/image_host/images/2020/7/1595063238.png)
...
![](https://cdn.jsdelivr.net/gh/xunhs/image_host/images/2020/7/1595063214.png)

## 效果

最终效果如图所示：

![](https://cdn.jsdelivr.net/gh/xunhs/image_host/images/2020/7/1595062840.png)
...
![](https://cdn.jsdelivr.net/gh/xunhs/image_host/images/2020/7/1595063186.png)
...
<!-- 插入图片 -->
![](https://cdn.jsdelivr.net/gh/xunhs/image_host/images/2020/7/1595063301.png)